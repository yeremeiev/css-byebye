    ...
    function filterRule (rule) {
      ...
      for (var j = 0, len = selectors.length; j < len; j++) {
        if (selectors[j].match(regex) === null) {
          selectorsToKeep.push(selectors[j])
        } else {
          selectorsToRemove.push(selectors[j])
        }
      }

      if (selectorsToKeep.length === 0) {
        rule.remove()
      } else if (selectorsToRemove.length) {
        if (selectorsToKeep.length === 1) {
          rule.selector = selectorsToKeep[0].trim()
        } else {
          selectorsToRemove.forEach(function(selector) {
            rule.selector = rule.selector.replace(new RegExp(selector + ',?[\\n\\s]*'), '')
          });  
        }
